...
function About() {
  ...
  // ===== 漢堡選單狀態 =====
  const [showMenu, setShowMenu] = useState(false); // 新增狀態
  ...
  const headerStyle = {
    display: 'flex',
    flexDirection: 'row', // 固定為 row，內部項目根據 isMobile 調整
    ...
    // 新增：手機版時，確保按鈕和選單圖標在右側，標題在左側
  };

  const navStyle = {
    display: isMobile ? 'none' : 'flex', // 手機版隱藏導覽列
    flexDirection: 'row',
    gap: '20px',
    fontWeight: 'bold',
    cursor: 'pointer',
  ...
  const authButtonsStyle = {
    display: isMobile ? 'none' : 'block', // 手機版隱藏登入/註冊按鈕
  };
  
  // 漢堡選單的樣式
  const mobileMenuStyle = {
      position: 'fixed',
      top: isMobile ? '70px' : '90px', // 調整為在 Header 下方
      right: 0,
      width: isMobile ? '60%' : '300px', // 手機版佔寬度 60%
  ...
  const mobileAuthContainerStyle = {
      display: 'flex',
      flexDirection: 'column',
      gap: '10px',
      marginTop: '15px',
  ...
  const burgerIconStyle = {
      ...
      cursor: 'pointer',
      color: '#6F4E37',
      display: isMobile ? 'block' : 'none', // 僅在手機版顯示漢堡圖示
  };
  ...
  const loginButton = {
    ...
    width: isMobile ? '100%' : 'auto', // 手機選單中按鈕佔滿
    marginLeft: isMobile ? '0' : '10px', // 手機選單中移除左邊距
  };
  ...
  const mainStyle = {
    ...
    // 調整 padding 以適應固定 header 
    padding: isMobile ? '80px 20px 50px' : '120px 50px 50px' 
  };
    ...
    <div style={containerStyle}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <header style={headerStyle}>
          {/* 網站標題 */}
          <div style={{ fontWeight: 'bold', fontSize: isMobile ? '1.5rem' : '2em', color: '#6F4E37', cursor: 'pointer' }} onClick={() => navigate('/')}>
            AI 履歷健診
          </div>
          ...
          <nav style={navStyle}>
            ...
            <div onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })}>聯絡我們</div>
          </nav>
          ...
          {/* 漢堡圖示 (僅手機版顯示) */}
          <div style={burgerIconStyle} onClick={() => setShowMenu(!showMenu)}>
            {showMenu ? '✕' : '☰'} 
          </div>
        ...
        {isMobile && (
            <div style={mobileMenuStyle}>
                <div style={mobileMenuItemStyle} onClick={() => handleNavClick(() => navigate('/'))}>首頁</div>
                <div style={mobileMenuItemStyle} onClick={() => handleNavClick(() => scrollToRef(aboutwe))}>關於我們</div>
                <div style={mobileMenuItemStyle} onClick={() => handleNavClick(() => scrollToRef(productRef))}>產品亮點</div>
                <div style={mobileMenuItemStyle} onClick={() => handleNavClick(() => scrollToRef(teamRef))}>團隊介紹</div>
                <div style={{...mobileMenuItemStyle, borderBottom: 'none'}} onClick={() => handleNavClick(() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }))}>聯絡我們</div>
                
                <div style={mobileAuthContainerStyle}>
                    <button style={loginButton} onClick={() => handleAuthClick(true)}>登入</button>
                    <button style={registerButton} onClick={() => handleAuthClick(false)}>註冊</button>
                </div>
            </div>
        )}
